...
SOURCE_DIR		:= src
BUILD_DIR		:= build
GUNICORN		:= /.pypy36-venv/bin/gunicorn
PYTHON			:= /.pypy36-venv/bin/pypy3
PIP				:= /.pypy36-venv/bin/pip3
PYPY36_INSTALL	:= /.pypy36-venv
DEBUG 			:= DEBUG=True

PYTHON_INCLUDE	:= $(shell python3.6-config --includes | sed s/-I/-isystem\ /g)
...
CC 				:= gcc
CPPFLAGS		+= -isystem $(PYPY36_INSTALL)/include -I . -I $(SOURCE_DIR) -I $(HTTP_PARSER_DIR)
CFLAGS			+= $(FEATURES) -std=c17 -fno-strict-aliasing -fcommon -fPIC -Wall -D DEBUG
LDFLAGS			+= -L$(PYPY36_INSTALL)/lib -L/usr/lib -L$(PYPY36_INSTALL)/lib_pypy -lpython3.6m -lpthread -ldl -lutil -lm -Xlinker -export-dynamic -Wl,-O1 -Wl,-Bsymbolic-functions -shared -fcommon
...
flask_bench 					:= bjoern/bench/flask_pypy36.txt
bottle_bench 					:= bjoern/bench/bottle_pypy36.txt
falcon_bench 					:= bjoern/bench/falcon_pypy36.txt
flask_gworker_bench 			:= bjoern/bench/flask_gworker_pypy36.txt
flask_valgrind			 		:= bjoern/bench/flask_valgrind_pypy36.mem
flask_callgrind			 		:= bjoern/bench/flask_callgrind_pypy36.mem
...
flask-ab: $(flask_bench) $(ab_post)
	...
	@echo -e "\n====== POST ======\n" | tee -a $(flask_bench)
	@echo -e "\n~~~~~ Keep Alive ~~~~~\n" | tee -a $(flask_bench)
	$(AB) -T 'application/x-www-form-urlencoded' -T 'Expect: 100-continue' -k -p $(ab_post) $(TEST_URL)
